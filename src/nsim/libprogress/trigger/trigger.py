...
class Trigger:
   ...
   def _identifier_validate (
      ...
      if ((not identifier) or (regenerate_identity)):
         identifier = app.libcommon.identifier.generate(
            owner=(owner or (
   ...
   def _trigger_bind_execute (
      ...
      try:
         ...
         if (identifier  != identifier_wait):
            app.libcommon.identifier.delete(identifier_wait)
      ...
      try:
         ...
         if (identifier  != identifier_block):
            app.libcommon.identifier.delete(identifier_block)
      ...
      try:
         if (reset):
            ...
            self._trigger_event_description  = (
               app.libcommon.descriptoroperations.combine(
                  self._trigger_event_description,
                  event_description,
               )
            )
            ...
            self._trigger_event_description  = (
               app.libcommon.descriptoroperations.remove(
                  event_description,
                  self._trigger_event_description,
               )
            )
      ...
      **error_values,
      ...
      try:
         if (reset):
            ...
            return (app.libcommon.descriptoroperations.check(
               error_description,
               self._error_description,
            ))
         elif (finalize):
            ...
            error_string          = 'ERROR:\n{0}\nValues:\n'.format(
               '\n'.join(
                  app.libcommon.descriptoroperations.extract(
                     self._error_description,
                  )
               ),
            )
